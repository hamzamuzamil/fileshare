...
export function ConnectionListItem({
  ...
  return (
    <div className="w-full mt-4 glass rounded-xl p-4">
        ...
        <div className="flex items-center space-x-2">
          <span className="text-sm font-medium text-white">
            {conn.browserName && conn.browserVersion ? (
              <>
                {conn.browserName}{' '}
                <span className="text-white/70">v{conn.browserVersion}</span>
              </>
            ) : (
              'Downloader'
            )}
          ...
          <span
            className={`px-2 py-1 text-white rounded-lg transition-colors duration-200 font-medium text-xs ${getStatusColor(
              conn.status,
            )}`}
          >
