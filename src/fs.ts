...
const getAsFile = (entry: any): Promise<UploadedFile> =>
...
const scanDirectoryEntry = async (entry: any): Promise<UploadedFile[]> => {
...
export const extractFileList = async (
  ...
  const scans: Promise<UploadedFile[]>[] = []
  const files: Promise<UploadedFile>[] = []
